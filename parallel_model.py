...
class ParallelModel(KM.Model):
    ...
    def __init__(self, keras_model, gpu_count, use_gpu=None):
        """Class constructor.
        ...
        if use_gpu and gpu_count != len(use_gpu):
            print ('Expect %d gpu no. but got %d instead, fallback to deafult.' %
                    (gpu_count, len(use_gpu)))
            use_gpu = None
        if not use_gpu:
            use_gpu = range(gpu_count)
        merged_outputs = self.make_parallel(use_gpu)
    ...
    def make_parallel(self, use_gpu):
        ...
        for i in range(self.gpu_count):
            with tf.device('/gpu:%d' % use_gpu[i]):
